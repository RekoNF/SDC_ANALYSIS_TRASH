...
def get_source(key):
    """Ambil sumber file: upload pengguna jika ada, kalau tidak pakai file default di ./data."""
    uploaded = st.session_state.get(f"upload_{key}")
    if uploaded is not None:
        return uploaded
    default_path = os.path.join(DATA_DIR, FILES[key])
...
with st.sidebar.expander("📤 Unggah data sendiri (opsional)", expanded=False):
    st.caption("Kosongkan jika ingin memakai data bawaan aplikasi.")
...
if missing:
    st.error(
        "File data berikut belum ditemukan: **" + ", ".join(missing) + "**.\n\n"
        "Silakan unggah lewat sidebar (📤 Unggah data sendiri), atau letakkan file tersebut "
        "di folder `data/` yang sejajar dengan `app.py`."
    )
    st.stop()
